...
const Sidebar = () => {
    ...
    const links = [
        { link: "/", label: "หน้าหลัก", icon: <BiHomeAlt2 className='w-6 h-6'/> },
        {
            link: "/student/tracks",
    ...
    return (
        <>
            <aside style={{ top: "65px" }} id="default-sidebar" className="border-r fixed top-16 left-0 z-40 w-72 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
                <div className="h-full px-3 py-4 lg:px-5 overflow-y-auto bg-white">
                    <ul className="font-medium space-y-1">
                        {links.map(link => (
                            <li key={link.link}>
                                <Link href={link.link}
                                    className={ url== link.link ? "bg-blue-500 text-white py-3 flex items-center p-2 rounded-lg hover:bg-blue-500 hover:text-white group" : "py-3 flex items-center p-2 text-gray-900 rounded-lg hover:bg-blue-500 hover:text-white group" }
                                >
                                    ...
                                    {link.icon}
                                    <span className="ml-3 font-bold">{link.label}</span>
                                </Link>
                            </li>
                        ))}
                        {/* <li>
