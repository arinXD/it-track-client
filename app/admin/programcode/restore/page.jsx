...
async function fetchDatas() {
    try {
        const result = await axios.get(`${hostname}/api/programcodes/getrestore`);
...
export default function RestoreProgramCode() {
    ...
    const handleRestore = async (id) => {
        ...
        const { value } = await swal.fire({
            text: `ต้องการคืนค่ารหัสหลักสูตร ${id.program_code} หรือไม่ ?`,
            icon: "question",
        ...
        if (value) {
            try {
                const result = await axios.post(`${hostname}/api/programcodes/restoreProgramCode/${id.id}`);
                const { ok, message } = result.data
                showToastMessage(true, `คืนค่ารหัสหลักสูตร ${id.program_code} สำเร็จ`)
    ...
    const filteredProgramcode = restores.filter(programcode => {
